...
default_thesaurus = os.path.join("data", "hpo_synonyms.txt")
umls_thesaurus = os.path.join("data", "hpo_umls_thesaurus.txt")
hpo_main_names = os.path.join("data", "hpo_term_names.txt")
...
def run_gui():
    ...
    def copy(button):
      ...
      if ClinPhen.getRadioButton("saveFormat") == "Comma-separated": pyperclip.copy(re.sub("\t", ",", terminalResults))
        ...
        pyperclip.copy(terminalResults)
      else: pyperclip.copy(terminalResults)
    ...
    def excel(button):
      saveLoc = os.path.join("misc", "Untitled.txt")
      global results
      saveResults = results
      if ClinPhen.getRadioButton("phenoLimit") == "Top 4": saveResults = "\n".join(saveResults.split("\n")[:5])
      writeFile = open(saveLoc, "w+")
